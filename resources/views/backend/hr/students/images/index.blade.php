		...
		<div class="forms">
			<div class="row">
				<div class="col-md-12">
					...
					<a href="{{route('student.images.create', $student)}}" class="btn btn-default"><i class="fas fa-plus-circle green-btn"></i>Add New Image</a>		
					...
					<table class="table table-striped table-bordered datatable" cellspacing="0"	>
						...
						@foreach($images as $image)
							<tr>
								...
								<td>									
									@if($image->status)
											<form action="{{route('student.images.update',[$student, $image])}}" method="POST" style="display: inline;">
												{{ csrf_field() }}
												{{ method_field('PUT') }}
									...
									<form action="{{route('student.images.destroy',[$student, $image])}}" method="POST" style="display: inline;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
